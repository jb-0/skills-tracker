...
import { PERMITTED_KEYWORDS, PERMITTED_LOCATIONS } from './constants';
...
const prepareQuery = (query: any) => {
  ...
  keywordsArray.forEach((keyword: any) => {
    if (PERMITTED_KEYWORDS.includes(keyword.toLowerCase())) {
      q.keywords += `${keyword} `;
    }
  });
  ...
  if (!PERMITTED_LOCATIONS.includes(q.locationName.toLowerCase())) q.locationName = 'london';

  // Encoded query
  const encodedQuery = `keywords=${q.keywords}&locationName=${q.locationName}&distanceFromLocation=${q.distanceFromLocation}`;
 ...
 */
export const searchReed = (query: any): Promise<any> => {
