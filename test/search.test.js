...
describe('Search Services', function () {
  describe('GET /api/jobs/search', function () {
    it('the number of jobs available is returned when the search route is called', async function () {
      const res = await request(app).get('/api/job/search').query({
    ...
    describe('prepareQuery()', function () {
      ...
      it('permitted locations are not replaced with london', function () {
        ...
        testCases.forEach(async (testCase) => {
          const expectedQuery = `keywords=react%20sql&locationName=${testCase}&distanceFromLocation=10`;
      ...
      it('distance must be an int, floats are converted to int and NaNs are defaulted to 10', function () {
        ...
        tests.forEach(async (test) => {
          ...
          assert.strictEqual(preparedQuery, `${baseExpectedQuery}${test.outcome}`);
        });
      });
    });
  });
  ...
  describe('POST /api/job/search/save', function () {
    describe('saveSearch() & pushSearchToUser()', function () {
      ...
      it('unauthenticated user cannot access the save search route', async function () {
        ...
        const res = await request(app).post('/api/job/search/save').send(search);

        assert.strictEqual(res.status, 302);
        assert.strictEqual(res.header.location, '/api/user/loginfailure');
      });

      it("new searches are saved in the Search collection and added to the user's savedSearches", async function () {
        const search = {
          keywords: 'react node',
          locationName: 'birmingham',
          distanceFromLocation: 20,
        ...
        const res = await request(app).post('/api/job/search/save').send(search).set('Cookie', USER_COOKIE);
      ...
      it('searches already saved by the user are not duplicated in the User collection', async function () {
        const search = {
          keywords: 'react node',
          locationName: 'birmingham',
          distanceFromLocation: 20,
      ...
      it("searches saved by other users can be added to a user's savedSearches", async function () {
        ...
        const res = await request(app).post('/api/job/search/save').send(search).set('Cookie', USER_2_COOKIE);
      ...
      it('searches saved by multiple users are not duplicated in the Searches collection', async function () {
        const filter = {
          keywords: 'node react',
          locationName: 'birmingham',
          distanceFromLocation: 20,
  ...
  describe('GET /api/jobs/search/trending', function () {
    it('up to three results are returned when the trending route is called', async function () {
      const res = await request(app).get('/api/job/search/trending');
  ...
  describe('DELETE /api/job/search/delete/:id', function () {
    ...
    describe('deleteUserSavedSearch()', function () {
      it("it is possible to delete a specific search if it exists in a user's saved searches", async function () {
        const res = await request(app).delete(`/api/job/search/delete/${savedSearchId}`).set('Cookie', USER_COOKIE);
      ...
      it('attempting to delete a search that does not exist throws resource not found', async function () {
        const res = await request(app).delete(`/api/job/search/delete/${savedSearchId}`).set('Cookie', USER_COOKIE);
      ...
      it('unauthenticated user cannot access the delete search route', async function () {
        const res = await request(app).delete(`/api/job/search/delete/${savedSearchId}`);
  ...
  describe('GET /api/job/search/saved', function () {
    describe('getUserSavedSearches()', function () {
      it('a user can get a full list of their saved searches', async function () {
        const res = await request(app).get('/api/job/search/saved').set('Cookie', USER_2_COOKIE);
      ...
      it('if a user has no saved searches the returned message states this', async function () {
        const res = await request(app).get('/api/job/search/saved').set('Cookie', USER_COOKIE);
